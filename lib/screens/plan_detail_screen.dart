...
class PlanDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final color = (plan['color'] as Color?) ?? const Color(0xFFFFD700);
    final imageUrl = plan['image']?.toString();
    final title = plan['title']?.toString() ?? "Plan";
    final amount = plan['amount']?.toString() ?? "₹0";
    final description = plan['description']?.toString() ?? "No description available.";
    final features = (plan['features'] is List) ? plan['features'] : <String>[];

    final isPurchased = purchasedPlans?.contains(title) ?? false;
    final bool isSubscribed = plan['status'] == true || isPurchased;
    ...
    return Scaffold(
      ...
      bottomNavigationBar:
      _buildBottomBar(context, color, isSubscribed, title),
    );
  }
    ...
    return Column(
      ...
      children: [
        ...
        _buildBenefitItem(
          Icons.trending_up,
          'Growth Potential',
          'Historical returns of ${plan['returns'] ?? 'N/A'}',
          color,
        ),
        _buildBenefitItem(
            ...
            children: [
              ...
              Text(
                "• Gold prices are subject to market fluctuations\n"
                    "• Minimum lock-in period: ${plan['minLockIn'] ?? '-'}\n"
  ...
  void _showEnrollmentDialog(
    ...
    final bankId = profileProvider
        .profileData?.data?.profile?.primaryBankAccount?.id;
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              final enrollProvider = context.read<EnrollInvestmentProvider>();
              await enrollProvider.enrollInvestment(body);
             print("eroll palns responce --------${enrollProvider.enrollResponse?.status}");
             if(enrollProvider.enrollResponse?.status=="success"){
              ScaffoldMessenger.of(context).showSnackBar(
               ...
               ScaffoldMessenger.of(context).showSnackBar(
                 SnackBar(
                   backgroundColor: Colors.green,
                   content: Text("Not Enrolled! ${enrollProvider.enrollResponse?.data}",
                       style: GoogleFonts.poppins()),
                 ),
               );
              }
